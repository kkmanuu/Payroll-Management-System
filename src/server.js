...
const signUp = async () => {
    ...
    const answers = await inquirer.prompt([
        ...
        { 
            ...
            validate: input => input.trim() ? true : 'Department name is required' 
        }
    ]);
    ...
    try {
        const [deptRows] = await db.query('SELECT department_id FROM departments WHERE department_name = ?', [department_name]);
        let department_id;
        ...
        if (deptRows.length > 0) {
            ...
            console.log(`New department "${department_name}" created with ID: ${department_id}`);
        }
        ...
        console.log('Sign up successful! You can now log in with your credentials.');
        return startScreen();
    } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
            console.log('Error: This email is already registered. Please use a different email.');
        } else {
...
const mainMenu = async (user) => {
    ...
    switch (action) {
        case 'Employee Management': return employeeManagement(user);
        case 'Department Management': return departmentManagement(user);
        case 'Leave Management': return leaveManagement(user);
        case 'Salary Management': return salaryManagement(user);
        case 'Record Attendance': return recordAttendance(user);
        case 'Generate Employee Report': return generateEmployeeReport(user);
        case 'Generate Department Report': return generateDepartmentReport(user);
        case 'Generate Leave Report': return generateLeaveReport(user);
        case 'Generate Salary Report': return generateSalaryReport(user);
        case 'Generate System User Report': return generateSystemUserReport(user);
        case 'View Audit Logs': return viewAuditLogs(user);
        case 'Logout':
    ...
    if (action === 'Add Employee') {
        ...
        const answers = await inquirer.prompt([
            ...
            { 
                ...
                validate: input => input > 0 ? true : 'Base salary must be a positive number',
                default: 0 
            },
            { 
                type: 'number', 
                ...
                validate: input => input >= 0 ? true : 'Deductions cannot be negative',
                default: 0 
            },
            { 
                type: 'input', 
                ...
                validate: input => /^\d{4}-\d{2}$/.test(input) ? true : 'Enter period in YYYY-MM format',
                default: '2025-04' 
            }
        ]);

        const { name, email, password, position, department_name, base_salary, deductions, period } = answers;
        ...
        try {
            ...
            await logAudit(user.employee_id, 'Add Employee', { name, email, department_name, base_salary, deductions, net_salary, period });
        ...
        try {
            const [rows] = await db.query('SELECT e.employee_id, e.employee_name, e.position, d.department_name FROM employees e LEFT JOIN departments d ON e.department_id = d.department_id');
            console.log('=== Employee List ===');
            rows.forEach(row => console.log(`ID: ${row.employee_id}, Name: ${row.employee_name}, Position: ${row.position}, Dept: ${row.department_name || 'N/A'}`));
        } catch (error) {
...
const departmentManagement = async (user) => {
    ...
    if (action === 'Add Department') {
        ...
        try {
            ...
            rows.forEach(row => console.log(`ID: ${row.department_id}, Name: ${row.department_name}`));
        } catch (error) {
...
const leaveManagement = async (user) => {
    const { action } = await inquirer.prompt({
        ...
        choices: ['Request Leave', 'Approve/Reject Leave', 'View Leaves', 'Back to Main Menu'],
    });
    ...
    if (action === 'Request Leave') {
        ...
        const { employee_id, date, type } = await inquirer.prompt([
            ...
            { type: 'input', name: 'type', message: 'Leave Type (e.g., Sick, Annual):' }
        ]);
        ...
        try {
            ...
            await logAudit(user.employee_id, 'Request Leave', { employee_id, date, type });
        ...
        const { leave_id, status } = await inquirer.prompt([
            ...
            { type: 'list', name: 'status', message: 'Status:', choices: ['Approved', 'Rejected'] }
        ]);

        try {
            await db.query('UPDATE leaves SET status = ? WHERE leave_id = ?', [status, leave_id]);
            await logAudit(user.employee_id, 'Update Leave Status', { leave_id, status });
        ...
        try {
            const [rows] = await db.query('SELECT l.leave_id, e.employee_name, l.leave_date, l.leave_type, l.status FROM leaves l JOIN employees e ON l.employee_id = e.employee_id');
            console.log('=== Leave List ===');
            rows.forEach(row => console.log(`ID: ${row.leave_id}, Name: ${row.employee_name}, Date: ${row.leave_date}, Type: ${row.leave_type}, Status: ${row.status}`));
        } catch (error) {
...
const salaryManagement = async (user) => {
    const { action } = await inquirer.prompt({
        ...
        choices: ['Add Salary', 'Calculate Payroll', 'View Payslip', 'Back to Main Menu'],
    });
    ...
    if (action === 'Add Salary') {
        ...
        const answers = await inquirer.prompt([
            { 
                ...
                name: 'employee_id', 
                message: 'Employee ID:', 
                validate: input => input > 0 ? true : 'Employee ID must be a positive number'
            },
            { 
                type: 'number', 
                name: 'base_salary', 
        ...
        try {
            ...
            const [employeeRows] = await db.query('SELECT employee_name FROM employees WHERE employee_id = ?', [employee_id]);
        ...
        try {
            ...
            await logAudit(user.employee_id, 'Calculate Payroll', { employee_id, period, net_salary });
...
const recordAttendance = async (user) => {
    ...
    try {
        ...
        await logAudit(user.employee_id, 'Record Attendance', { employee_id, date, hours, overtime });
...
const generateDepartmentReport = async (user) => {
    try {
        const [rows] = await db.query('SELECT d.department_name, COUNT(e.employee_id) as employee_count FROM departments d LEFT JOIN employees e ON d.department_id = e.department_id GROUP BY d.department_id');
        console.log('=== Department Report ===');
        rows.forEach(row => console.log(`Department: ${row.department_name}, Employees: ${row.employee_count}`));
...
const generateLeaveReport = async (user) => {
    try {
        const [rows] = await db.query('SELECT e.employee_name, l.leave_date, l.leave_type, l.status FROM leaves l JOIN employees e ON l.employee_id = e.employee_id');
        console.log('=== Leave Report ===');
        rows.forEach(row => console.log(`Name: ${row.employee_name}, Date: ${row.leave_date}, Type: ${row.leave_type}, Status: ${row.status}`));
...
const generateSalaryReport = async (user) => {
    try {
        const [rows] = await db.query('SELECT e.employee_name, s.base_salary, s.deductions, s.net_salary, s.period FROM salaries s JOIN employees e ON s.employee_id = e.employee_id');
        console.log('=== Salary Report ===');
        rows.forEach(row => console.log(`Name: ${row.employee_name}, Period: ${row.period}, Gross: ${row.base_salary}, Deductions: ${row.deductions}, Net: ${row.net_salary}`));
...
const generateSystemUserReport = async (user) => {
    try {
        const [rows] = await db.query('SELECT employee_id, employee_name, email_id, position FROM employees');
        console.log('=== System User Report ===');
        rows.forEach(row => console.log(`ID: ${row.employee_id}, Name: ${row.employee_name}, Email: ${row.email_id}, Position: ${row.position}`));
...
const viewAuditLogs = async (user) => {
    try {
        const [rows] = await db.query('SELECT l.log_id, e.employee_name, l.action, l.timestamp, l.details FROM audit_logs l JOIN employees e ON l.user_id = e.employee_id');
        console.log('=== Audit Logs ===');
        rows.forEach(row => console.log(`ID: ${row.log_id}, User: ${row.employee_name}, Action: ${row.action}, Time: ${row.timestamp}, Details: ${row.details}`));
    } catch (error) {
